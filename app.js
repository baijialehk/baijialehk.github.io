/**
 * 百家乐路纸推演系统 - 终极完整版 V9
 * 新增：单跳龙、隔山打牛、蜻蜓点水、大眼仔连续蓝点、小路三连红蓝、小路断层
 */
 
// ==================== 系统密码 ====================
const SYSTEM_PWD = '百家乐路纸推演系统';
    ...
    if (votes) {
        ...
        [1, 2, 3].forEach(tier => {
            ...
            if (tierVotes.length >= 0) {
                const tierName = tier === 1 ? '大数据触发' : (tier === 2 ? '实时信号触发' : '赌徒直觉触发');
...
function submitResult() {	
	...
	document.getElementById('loading-overlay').style.display = 'flex';   //显示等待画面
...
/*
	...
	fetch('http://asd.d.gv.vc:3456/api/predict', {
		method: 'POST',
	...
	.then(response => {
		if (!response.ok) {
			//throw new Error('服务端返回错误: ' + response.status);
				document.getElementById('vote-detail').textContent = ' 🔶服务端返回错误: '+ response.status + ' 请检查网络';
		}
	...
	.then(predResult => {
		...
		// ... 全部保留 ...
			document.getElementById('loading-overlay').style.display = 'none';  //隐藏等待画面，恢复界面
						//console.log('✅ 服务端计算成功:', predResult.prediction, predResult.source);
			document.getElementById('vote-detail').textContent = ' ✅ 结果计算成功 ';
			render(predResult);
	})
	.catch(err => {
			document.getElementById('loading-overlay').style.display = 'none';  //隐藏等待画面，恢复界面
				document.getElementById('vote-detail').textContent = ' 🔶远端调用失败: ' + err + ' 请检查网络';
...
function init() {
	...
	document.getElementById('btn-login').addEventListener('click', function() {
		...
		fetch('http://asd.d.gv.vc:3456/api/login', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
		...
		.then(data => {
			if (data.success) {
				...
				const levelMessages = {
					0: '🆓 免费账号 | ⚠️每轮有限定局数，无法修改配置，没有配备大数据库，只能靠自己手动积累。\n',
					1: '👑 高级用户 | 无限牌局 + 自定义配置 + 更多种类的智能预测！拥有10万局真实牌局的经验记录，涵盖所有形态与机率！/n',
					2: '🎰 高级用户 | 能追踪各种形态的连续输赢情况，让你在任何牌型都能把握先机！/n',
					3: '🔧 高级用户 | 能结合你的经验与偏好配置各种信号的票数权重，让结果更加精准，这才是秘笈！\n',
					4: '🛡️ 想无限畅玩、解锁全部信号权重？升级用户，尊享顶级推演体验！/n',
				};
				const levelMessage = `✅ 登录成功 | 账号类型: ${data.level}` + levelMessages[data.level] || '';
				document.getElementById('vote-detail').textContent = levelMessage;
				...
				//document.getElementById('btn-submit').textContent = '✅ 确定';   // 提交的确认按钮
				startApp();
			} else {
	...
	document.getElementById('btn-register').addEventListener('click', function() {
		...
		fetch('http://asd.d.gv.vc:3456/api/register', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
		...
		.then(data => {
			if (data.success) {
				document.getElementById('login-status').textContent = '✅ 注册成功，请登录';
			} else {
   ...
   //===========================================
	...
	function startApp() {
		...
		if (isIndex) {
			...
			document.getElementById('val-resonance').textContent = '否';
			document.getElementById('final-judgment').textContent = '等待录入…';
			document.getElementById('final-judgment').className = 'final-judgment';
			//document.getElementById('winrate-percent').textContent = '0%';
